...
def get_onnx_network(network, batch_size):
    netname = network.split('/')[1]
    if netname == 'resnet-50':
        onnx_url = 'https://s3.amazonaws.com/download.onnx/models/opset_9/resnet50.tar.gz'
    ...
    mpath = onnx_path.replace(".tar.gz", "/model.onnx")
...
def load_and_transform_image(input_f, network_name, input_shape):
    if network_name.startswith('onnx/'):
        netname = network.split('/')[1]
    ...
    # This is the standard imagenet normalization, right?
